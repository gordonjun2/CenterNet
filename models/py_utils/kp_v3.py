...
from .utils_v3 import convolution, residual
from .utils_v3 import make_layer, make_layer_revr, make_hg_layer#, make_up_layer            # Added make_hg_layer

from .kp_utils_v3 import _tranpose_and_gather_feat, _decode
from .kp_utils_v3 import _sigmoid, _ae_loss, _regr_loss, _neg_loss
from .kp_utils_v3 import make_tl_layer, make_br_layer, make_kp_layer, make_ct_layer
from .kp_utils_v3 import make_pool_layer, make_unpool_layer
from .kp_utils_v3 import make_merge_layer, make_inter_layer, make_cnv_layer

class kp_module(nn.Module):
    def __init__(
        self, n, dims, modules, layer=residual,
        make_up_layer=make_layer, make_low_layer=make_layer,             # Let make_up_layer = make_hg_layer
        ...
        #self.up1  = make_up_layer(curr_mod, curr_dim)                                 # Dense block does not need images' dimensions
        ...
        self.max1 = make_pool_layer(curr_dim)                                           # CONSIDER REMOVING
        ...
        #make_low_layer(kernel =3, mod = next_mod, dim0 = next_dim, dim1 = next_dim, nDenselayer = 1)
        ...
        #self.low3 = make_hg_layer(kernel = 3, mod = curr_mod, dim0 = next_dim, dim1 = curr_dim, nDenselayer = 1)
        self.low3 = make_hg_layer_revr(
            3, next_dim, curr_dim, curr_mod,
            layer=layer, **kwargs
        )
    ...
    def forward(self, x):
        #print("Print x size:" + str(x.size()))
        up1  = self.up1(x)
        #print("Print up1 size:" + str(up1.size()))
        max1 = self.max1(x)
        #print("Print max1 size:" + str(max1.size()))
        low1 = self.low1(max1)
        #print("Print low1 size:" + str(low1.size()))
        low2 = self.low2(low1)
        #print("Print low2 size:" + str(low2.size()))
        low3 = self.low3(low2)
        #print("Print low3 size:" + str(low3.size()))
        up2  = self.up2(low3)
        #print("Print up2 size:" + str(up2.size()))
...
class kp(nn.Module):
    ...
    def _train(self, *xs):
        ...
        #print("Print image size:" + str(image.size()))

        inter      = self.pre(image)
        #print("Print inter size:" + str(inter.size()))
        outs       = []
